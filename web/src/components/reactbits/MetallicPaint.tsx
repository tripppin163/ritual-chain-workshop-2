 ...
 * MIT licensed and published to be copied into a project rather than installed.
 *
 * It takes a silhouette rather than a colour: every pixel that is neither near-white
 * nor transparent becomes the shape, a Laplace solve over that shape gives it depth,
 * and the shader flows inside it. So a button has to hand it its own outline.
 *
 * Local changes: the "use client" directive Next needs; optional-index guards so it
 * typechecks under this project's noUncheckedIndexedAccess; a drawing buffer sized to
 * the element instead of a fixed 1000x1000 square; and an aspect-independent slope term
 * in the fragment shader. Both of the latter are the same bug from two directions:
 * upstream assumes a square frame, and a wide button makes it letterbox the artwork and
 * tear one colour channel loose. Details at each site.
 */
...
void main(){
  ...
  sc=clamp(sc,0.,1.);
  // sl is fed to a per-channel softness term as 1.-sl, which upstream assumes stays
  // positive because sc.x only reaches 1 on a square element. sc.x carries the aspect
  // ratio, so on a wide control the divisor collapses, the green channel smooths out
  // while red and blue keep banding, and the shape grows a magenta streak. Taking the
  // slope from unscaled coordinates keeps the intended range on any aspect.
  float sl=vP.x-(1.-vP.y),an=u_time*.001;
  vec2 iC=fA();
  ...
  // The three channels are banded at different softnesses, which is where dispersion
  // comes from — but upstream applies it whatever u_chroma says, so a chromaticSpread of
  // 0 still leaves green washed out against red and blue and blooms violet at the corner
  // where the divisor below collapses. Scaling the per-channel offsets by u_chroma makes
  // 0 mean neutral metal and leaves any positive value behaving as before.
  float rC=mG(hi.r,lo.r,rP,sf+u_chroma*(.018+u_refract*cv*.025),cv);
  float gP=fract(fl);
  float gC=mG(hi.g,lo.g,gP,sf+u_chroma*(.008/max(.01,1.-sl)),cv);
  float bP=fract(fl-bO);
  float bC=mG(hi.b,lo.b,bP,sf+u_chroma*.008,cv);
...
export default function MetallicPaint({
  ...
  const uploadTexture = useCallback((imgData: ImageData) => {
    ...
    gl.uniform1f(uniforms.u_ratio ?? null, boxRatioRef.current);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const w = Math.max(1, Math.round((canvas.clientWidth || 1000) * dpr));
      const h = Math.max(1, Math.round((canvas.clientHeight || 1000) * dpr));
      if (canvas.width !== w || canvas.height !== h) {
        canvas.width = w;
        canvas.height = h;
      }
      gl.viewport(0, 0, w, h);
      boxRatioRef.current = w / h;
      const u = uniformsRef.current;
      if (u.u_ratio) gl.uniform1f(u.u_ratio, boxRatioRef.current);
    };
